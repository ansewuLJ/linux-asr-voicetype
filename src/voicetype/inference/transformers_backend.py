...
class TransformersBackend:
    ...
    def load(self) -> None:
        ...
        self._model = Qwen3ASRModel.from_pretrained(self._config.model, **model_kwargs)
        LOGGER.info("Loaded transformers backend model=%s dtype=%s", self._config.model, self._config.dtype)
    ...
    def transcribe(self, samples: np.ndarray, language: str | None, context: str | None) -> AsrResult:
        ...
        kwargs: dict[str, object] = {"audio": (samples, 16000)}
        if language:
    ...
    def _prepare_hf_env(self) -> None:
        ...
        os.environ.setdefault("HF_HUB_ETAG_TIMEOUT", str(self._config.hf_hub_etag_timeout_sec))
        os.environ.setdefault("HF_HUB_DOWNLOAD_TIMEOUT", str(self._config.hf_hub_download_timeout_sec))
    ...
    def _model_repo_reachable(self) -> bool:
        ...
        endpoint = self._config.hf_endpoint or os.environ.get("HF_ENDPOINT") or "https://huggingface.co"
